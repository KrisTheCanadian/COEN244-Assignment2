...
Room::Room(int s_num, int m_room) : m_bed_number(s_num), m_room(m_room) {
	for (int i = 0; i < 12; i++) { for (int j = 0; j < 30; j++) { m_reserve[i][j] = 0; } }
}
...
int Room::get_reservation(int day, int month) { return m_reserve[month-1][day-1]; } //offset of 1 for day and month (since array starts at 0)
void Room::remove_reservation(int month, int day) { m_reserve[month-1][day-1] = 0; }
void Room::display() {
